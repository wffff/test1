...
public class GoodsSpecEntity extends AbstractEntity {
    ...
    @OneToMany(cascade = CascadeType.ALL,mappedBy = "spec",fetch = FetchType.EAGER)
    private List<MemberInventoryEntity> memberInventoryList = new ArrayList<>();

    public GoodsSpecEntity() {
    }

    public GoodsSpecEntity(Integer model, String name, Double price, Double marketPrice, Double discount,
